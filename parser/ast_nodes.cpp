...
void parser::Ast_Node_Code_Block::set_code(Ast* __ast, bool __once = 0) {
    ...
    while(__ast->tokens_position < __ast->tokens_collection->count) {
        ...
        switch (get_node_type(__ast))
        {
            ...
            case AST_NODE_EXPRESSION: case AST_NODE_VARIABLE: case AST_NODE_FUNCTION_CALL: case AST_NODE_IMPLICIT_VALUE: 
...
parser::Ast_Node_Variable_Declaration::Ast_Node_Variable_Declaration(Type_Information* __type, Token* __variable_token_name, bool __is_static)
    : Ast_Node(AST_NODE_VARIABLE_DECLARATION, __type), variable_token_name(__variable_token_name), is_static(__is_static), constructor_call(0) {}
...
parser::Ast_Node_Expression::Ast_Node_Expression(Ast_Node* __value, Token* __token_id) : Ast_Node(AST_NODE_EXPRESSION, 0) 
    { values = new utils::Linked_List <Ast_Node*>(); token_ids = new utils::Linked_List <Token*>(); token_ids->destroy_content = 0; values->add(__value); token_ids->add(__token_id); }

parser::Ast_Node_Expression::Ast_Node_Expression(Ast_Node* __value, Ast_Node* __value_2, Token* __token_id) : Ast_Node(AST_NODE_EXPRESSION, 0) 
    { values = new utils::Linked_List <Ast_Node*>(); token_ids = new utils::Linked_List <Token*>(); token_ids->destroy_content = 0; values->add(__value); values->add(__value_2); token_ids->add(__token_id); }
...
void parser::Ast_Node_Expression::set_representive_type(Ast* __ast) {
    ...
    if (_token_ids->count == 1 && _values->count == 1) {
        ...
        _values->getDataLinkedList(0)->object = _function_declaration_node;
...
parser::Ast_Node_Constructor_Call::Ast_Node_Constructor_Call(Type_Information* __representive_type, char* __search_name)
    : Ast_Node(AST_NODE_CONSTRUCTOR_CALL, __representive_type), this_variable(0), search_name(__search_name) {
...
void parser::Ast_Node_Constructor_Call::set_variable_declaration_constructor(Ast* __ast, Ast_Node_Expression* __expression) {
    ...
    Type_Information* _struct_declaration_type = 
        ( (Ast_Node_Variable_Declaration*) __ast->open_nodes->last->object)->representive_type->get_copy();
    ...
    ( (Ast_Node_Variable_Declaration*) __ast->open_nodes->last->object )->constructor_call =
...
parser::Ast_Node_Cast::~Ast_Node_Cast() 
    { constructor_call->~Ast_Node_Constructor_Call(); free(constructor_call); value->~Ast_Node(); free(value); }
...
void parser::Ast_Node_Return_Key_Word::confirm_return_type(Ast* __ast, int __inicial_position) {
    ...
    for (int _  = 0; _ < __ast->open_nodes->count && !_function_declaration; _++)
...
void parser::Ast_Node_Return_Key_Word::set_expression(Ast* __ast) 
    { expression = Ast_Node_Expression::generate(__ast); expression->set_representive_type(__ast); representive_type = expression->representive_type; }
...
parser::Ast_Node_If::Ast_Node_If(Ast_Node_Code_Block* __body) : Ast_Node(AST_NODE_IF, 0), body(__body) {}
...
parser::Ast_Node_Else_If::Ast_Node_Else_If(Ast_Node_Code_Block* __body) : Ast_Node(AST_NODE_ELSE_IF, 0), body(__body) {}
...
parser::Ast_Node_For::Ast_Node_For(Ast_Node_Code_Block* __body) : Ast_Node(AST_NODE_FOR, 0), body(__body) {}
